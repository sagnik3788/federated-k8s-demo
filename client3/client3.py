...
class MobileNetClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        return model.get_weights(), len(train_generator), {"model_type": "mobilenet"}

    def evaluate(self, parameters, config):
        model.set_weights(parameters)
        loss, accuracy = model.evaluate(train_generator, steps=len(train_generator))
        return loss, len(train_generator), {"accuracy": accuracy, "model_type": "mobilenet"}

# Start Flower client using the new start_client method
fl.client.start_client(
    server_address="localhost:8080",
    client=MobileNetClient().to_client(),  # Convert NumPyClient to Client
)